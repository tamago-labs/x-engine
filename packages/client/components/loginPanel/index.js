    ...
    return (
        <div className="my-auto">
            ...
            {!isConnected && (
                <button onClick={() => {
                    redirectToAuthUrl()
                }} className={`inline-flex  justify-center gap-x-1.5 rounded bg-white px-6 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 `}>
...
export const LoginButton = () => {
    ...
    const { redirectToAuthUrl } = useContext(AccountContext)

    return (
        <button onClick={() => {
            redirectToAuthUrl()
        }} className={`inline-flex  justify-center gap-x-1.5 rounded bg-white px-6 py-1.5 text-base font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 `}>
            Login <span className="hidden md:inline-block">with Google</span>
        </button>
    )
}
